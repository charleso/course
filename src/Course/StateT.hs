...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f sa = StateT $ \s -> (\(a, s1) -> (f a, s1)) <$> runStateT sa s
...
instance Bind f => Apply (StateT s f) where
  (<*>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  -- (<*>) sab sa = StateT $ \s -> (runStateT sab s) >>= (\(ab, s1) -> (\(a, s2) -> (ab a, s2))  <$> runStateT sa s1)
  (<*>) sab sa = StateT $ \s -> (runStateT sab s) >>= \(ab, s1) -> runStateT (ab <$> sa) s1
...
instance Monad f => Bind (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (=<<) afb fa = StateT $ \s -> runStateT fa s >>= \(a, s1) -> runStateT (afb a) s1
...
distinct' ::
  ...
  List a
  -> List a
-- QUESTION: Is this supposed be different?
distinct' = distinct
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF = (flip evalT S.empty) . filtering (\x -> do
  set <- if x > 100 then StateT (const Empty) else getT
  if S.member x set then return False else (putT (S.insert x set) >> return True))
...
emptyOT :: Applicative f => OptionalT f a
emptyOT = OptionalT (pure Empty)
...
instance Monad f => Bind (OptionalT f) where
  (=<<) oab ofa = OptionalT $ (runOptionalT ofa) >>= runOptionalT . (foldOptional oab emptyOT)
...
instance Bind (Logger l) where
  (=<<) falb (Logger l a) = let (Logger l2 b) = falb a in Logger (l ++ l2) b
...
distinctG = runOptionalT . (flip evalT S.empty) . filtering (\x -> do
  set <- if x > 100
    then StateT $ const $ OptionalT $ log1 ("aborting > 100: " ++ (show' x)) Empty
    else if even x then (StateT $ \s -> OptionalT $ log1 ("even number: " ++ (show' x)) (Full (s, s))) else getT
  if S.member x set then return False else (putT (S.insert x set) >> return True))
