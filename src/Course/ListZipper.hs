...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p lz@(ListZipper _ a _) = if p a then IsZ lz else findLeft p -<< moveLeft lz
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p lz@(ListZipper _ a _) = if p a then IsZ lz else findRight p -<< moveRight lz
...
moveLeftLoop (ListZipper (h :. t) a r) = ListZipper t h (a :. r)
...
moveRightLoop (ListZipper l a (h :. t)) = ListZipper (a :. l) h t
...
moveLeft (ListZipper (h :. t) a r) = IsZ $ ListZipper t h (a :. r)
...
moveRight (ListZipper l a (h :. t)) = IsZ $ ListZipper (a :. l) h t
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper (h :. t) a r) = IsZ (ListZipper (a :. t) h r)
swapLeft _                         = IsNotZ
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper l a (h :. t)) = IsZ (ListZipper l h (a :. t))
swapRight _                         = IsNotZ
...
moveLeftN n lz = if n > 0 then (moveLeftN (n - 1)) -<< (moveLeft lz)
                          else (moveLeftN (n + 1)) -<< (moveRight lz)
...
moveLeftN' n lz = foldOptional Right (Left . length . (if n > 0 then lefts else rights) $ lz) (toOptional . moveLeftN n $ lz)
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l a r) = let (h :. t) = reverse (a :. r) in ListZipper (t ++ l) h Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l a r) = let (h :. t) = reverse (a :. l) in ListZipper Nil h (t ++ r)
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a (ListZipper l m r) = ListZipper (m :. l) a r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a (ListZipper l m r) = ListZipper l a (m :. r)
...
instance Apply ListZipper where
  (<*>) (ListZipper l1 a1 r1) (ListZipper l2 a2 r2) = ListZipper (zipWith ($) l1 l2) (a1 a2) (zipWith ($) r1 r2)
...
instance Extend ListZipper where
  (<<=) f lz = f <$> ListZipper (initsLZ moveLeft lz) (lz) (initsLZ moveRight lz)

initsLZ :: (ListZipper a -> MaybeListZipper a) -> ListZipper a -> List (ListZipper a)
initsLZ move = unfoldr $ (split <$>) . toOptional . move
  where split a = (a, a)
...
instance Traversable ListZipper where
  traverse afb (ListZipper l m r) = ListZipper <$> traverse afb l <*> afb m <*> traverse afb r
