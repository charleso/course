...
noneof ::
  Chars
  -> Parser Char
noneof s = satisfy (flip notElem s)
...
hex ::
  Parser Char
hex = flbindParser (thisMany 4 (satisfy isHexDigit)) $ foldOptional (valueParser . chr) failed . readHex
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll lcb = character >>= \c ->
  if and (sequence lcb c) then valueParser c else unexpectedCharParser c
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny lcb = character >>= \c ->
  if or (sequence lcb c) then valueParser c else unexpectedCharParser c
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma l r pa = betweenCharTok l r (sepby pa (charTok ','))
